...
final class LaneNormalizationTests: XCTestCase {
    ...
    func testDrumMachineKickMappings() {
        XCTAssertEqual(ChartDocument.laneIndex(forPipelineLane: "808"), Lane.kick.rawValue)
        XCTAssertEqual(ChartDocument.laneIndex(forPipelineLane: "909"), Lane.kick.rawValue)
        XCTAssertEqual(ChartDocument.laneIndex(forPipelineLane: "bass drum"), Lane.kick.rawValue)
        XCTAssertEqual(ChartDocument.laneIndex(forPipelineLane: "kick drum"), Lane.kick.rawValue)
    }
    ...
    func testChartLoadingWithCamelCaseLanes() {
        ...
        let notes: [NoteEvent] = [
            NoteEvent(id: UUID(), lane: .kick, time: 0.0, label: "Kick"),
        ...
        XCTAssertEqual(displayLanes.map { $0.sourceLane }, [.kick, .red, .yellow, .blue, .green])
    }

    func testMixedLaneNamesInChart() {
        // Some notes might have explicit labels, others use lane defaults
        let notes: [NoteEvent] = [
            NoteEvent(id: UUID(), lane: .kick, time: 0.0, label: "808 Bass"),
